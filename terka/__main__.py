...
from terka.strike_defaults import default_for_action, for_action_with_contact
...
def convert(video: Path, model_path: Path | None,
    ...
    strike_params = (
        for_action_with_contact(meta.action, samples) if meta else None
    )
...
def ingest(rgb_root: Path, model_path: Path | None,
    ...
    with ExitStack() as stack:
        ...
        for i, meta in enumerate(iter_dataset(rgb_root)):
            ...
            doc = trajectory_doc(
                samples,
                strike_params=for_action_with_contact(
                    meta.action, samples),
                extra={
                "thetis": {
                    "subject_num": meta.subject_num,
